...
sub get_basic
{
	...
	my $sess_dt = "";
	if ($ENV{DISPLAY}) {
		$sess_type = $ENV{XDG_SESSION_TYPE};
		$sess_dt = $ENV{DESKTOP_SESSION};
	}

	$info{sess_type} = $sess_type;
	$info{sess_dt} = $sess_dt;
...
sub get_cpu_mem
{
    ...
    $mem_info{mem_total} = format_size($mem_total, 'KB');
    $mem_info{mem_free} = format_size($mem_free, 'KB');
    $mem_info{mem_avail} = format_size($mem_avail, 'KB');
    $mem_info{swap_total} = format_size($swap_total, 'KB');
    $mem_info{swap_free} = format_size($swap_free, 'KB');
...
sub get_virtual
{
    ...
    if ($virt eq 'kvm' && -f '/sys/devices/virtual/dmi/id/product_name') {
        open my $fh, '<', '/sys/devices/virtual/dmi/id/product_name';
        my $name = <$fh>; chomp $name; close $fh;
        $virt .= " ($name)";
    } elsif ($virt eq 'xen' && -f '/sys/devices/virtual/dmi/id/product_version') {
        open my $fh, '<', '/sys/devices/virtual/dmi/id/product_version';
...
sub get_security
{
    ...
    my $fwall = system('systemctl status firewalld > /dev/null 2>&1') == 0 ? 'Running' : 'Not Running';
    return ($secur, $fwall);
}
...
sub get_drives
{
    ...
    foreach my $ln (@z) {
        ...
        $work = sprintf "%-5s %-5s %-6s %-5s %s", $parts[1], $parts[2], $parts[3], $parts[4], $mtpt;
        push @drives, $work;
    }
