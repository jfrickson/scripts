...
function sk
{
	...
	if [[ -z "$1" ]]; then
		printf "\n${_dr}Specify a session id$ to kill${_rst}\n"
		return
	fi
	...
	while [[ -n "$1" ]]; do
		...
		if [ ! -S "$sock" ]; then
			...
			local pid=$(fuser "$sock" 2>/dev/null | awk '{print $NF}')
			if [ -n "$pid" ]; then
...
function sl
{
	...
	sessions=($(ls -1 "$DTACH_DIR" 2>/dev/null))
...
{
    if [ -z "$1" ]; then
        printf "\n${_dr}You must specify a session id${_rst}\n"
		return
	fi
	...
	if [ -n "$DTACH_SESSION" ]; then
		printf "\n${_dr}Already in a dtach session. Please exit or detach first.${_rst}\n\n"
		return
	elif [ -S "$sock" ]; then
	...
	if [ -f "$DTACH_DIR/.next_session" ]; then
		local next=$(<"$DTACH_DIR/.next_session")
...
function sw
{
	...
	case "$id" in
		+)	id=$(ls -1 "$DTACH_DIR" 2>/dev/null | grep -A1 "^$DTACH_SESSION$" | tail -n1)
			if [[ -z "$id" || "$id" = "$DTACH_SESSION" ]]
				then id=$(ls -1 "$DTACH_DIR" 2>/dev/null | head -n1); fi
			if [[ -z "$id" || "$id" = "$DTACH_SESSION" ]]; then
				printf "%sNo next session found%s\n" "$_dy" "$_rst"
				return
			fi
			;;
		-)	id=$(ls -1 "$DTACH_DIR" 2>/dev/null | grep -B1 "^$DTACH_SESSION$" | head -n1)
			if [[ -z "$id" || "$id" = "$DTACH_SESSION" ]]
				then id=$(ls -1 "$DTACH_DIR" 2>/dev/null | tail -n1); fi
			if [[ -z "$id" || "$id" = "$DTACH_SESSION" ]]; then
